import { Patient, Gender } from './types/fullPatientEntry';
...
import fullpatientServices from './services/fullpatientServices';
...
app.get('/patients/full/:id', (req, res) => {
    const patient = fullpatientServices.findById((req.params.id));
    if (patient) {
      res.send(patient);
    } else {
      res.sendStatus(404);
  ...
  app.get('/api/diagnoses/:code', (req, res) => {
    const diagnose = diagnosesService.findByCode((req.params.code));
